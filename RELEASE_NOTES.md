...
- `+Z`：构建号（**全局递增**，每次构建 APK 时递增，**不按版本重置**）

**版本号递增规则：**
- `Y` 递增：新增功能、优化、bug 修复
- `X` 递增：重大变更、不兼容更新
- `Z` 递增：每次构建 APK 时递增（必须始终大于上一次构建的 `+Z`）
  - **重要**：Android 系统仅用 `+Z` 判断能否覆盖安装，**必须全局递增，不能重置为 1**
  - 例如：`v2.0+2` 的下一次构建应该是 `v2.1+3`，不是 `v2.1+1`
...
**`+Z` 说明：** 构建 workflow 会自动从 GitHub Releases 读取上一次的 `versionCode` 并递增，无需手动维护。`pubspec.yaml` 中的 `+Z` 仅作为参考值，实际构建时会被替换。
